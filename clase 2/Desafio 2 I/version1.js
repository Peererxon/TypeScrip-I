var Tren = /** @class */ (function () {
    ...
    Tren.prototype.pintar = function (contexto, cordx, cordy, figura, radio, dibx, diby) {
        /*let color = "rgb("+Math.random()*255+","+Math.random()*255+","+
        Math.random()*255+");"*/
        var color = "rgb(" + Math.ceil(Math.random() * 255) + "," + Math.ceil(Math.random() * 255) + "," +
            Math.ceil(Math.random() * 255) + ")";
        contexto.fillStyle = color;
        switch (figura) { // Circulo
            case 0:
                contexto.arc(cordx, cordy, dibx, diby, Math.PI * 2);
                contexto.fill();
                contexto.stroke();
            case 1: // Triangulo
                for (var i = 1; i < 3; i++) {
...
if (canvas != undefined && canvas.getContext) {
    tren.pintar(contexto, 50, 500, 0, 10, 1, 1);
}
else

function calcsueldo(sueldobase, años, hijos) {
    if (años === void 0) { años = 1; }
...
for (var i = 1; i < n; i++) {
    var nombre = "Pedro";
    var sueldobase = 1 + Math.round(Math.random() * 9000);
    var años = 1 + Math.round(Math.random() * 33);
...
function solicitar_datos() {
    var n = parseInt(prompt("Ingrese el numero de los trabajadores: "));
    if (!isNaN(n)) {
        for (var i = 1; i <= n; i++) {
            var name = prompt("introduzca su nombre por favor");
